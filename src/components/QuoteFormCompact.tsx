...
export default function QuoteFormCompact() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await submitForm("quote", {
      formType: "Instant quote (compact)",
    ...
    <form onSubmit={submit} className="rounded-2xl bg-white p-5 md:p-6 shadow-2xl border border-slate-200">
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input name="pickupZip" required className="form-input" placeholder="46373" />
        </div>
        <div>
          <label className="form-label">Delivery ZIP</label>
          <input name="deliveryZip" required className="form-input" placeholder="75201" />
        </div>
        <div className="sm:col-span-2">
          <label className="form-label">Equipment Type</label>
          <select name="equipment" required className="form-select" defaultValue="">
        ...
        <div>
          ...
          <input name="weight" className="form-input" placeholder="42,000" />
        </div>
        <div>
          <label className="form-label">Pickup Date</label>
          <input name="pickupDate" type="date" className="form-input" />
        </div>
        <div className="sm:col-span-2">
        ...
        <div className="sm:col-span-2">
          ...
          <input name="phone" required type="tel" className="form-input" placeholder="(555) 555-1234" />
        </div>
      </div>
      {error && (
